...
if not isinstance(deployment, dict):
    raise SystemExit("Health check failed: response is missing deployment")

runtime_build_revision = str(deployment.get("runtime_build_revision") or "").strip()
runtime_build_revision_state = str(deployment.get("runtime_build_revision_state") or "").strip()
if runtime_build_revision_state == "runtime_build_revision_missing":
    raise SystemExit("Health check failed: deployment runtime_build_revision is still missing.")
if not runtime_build_revision:
    raise SystemExit("Health check failed: deployment runtime_build_revision is empty.")

local_repo_head_sha = os.popen("git rev-parse HEAD").read().strip()
if not local_repo_head_sha:
    raise SystemExit("Health check failed: local repo HEAD could not be resolved for deploy parity.")
if runtime_build_revision != local_repo_head_sha:
    raise SystemExit(
        "Health check failed: deployment runtime_build_revision "
        f"{runtime_build_revision!r} does not match local repo HEAD {local_repo_head_sha!r}."
    )

if deployment_evidence_checked:
    evidence_after_sha = str(evidence.get("after_sha") or "").strip()
    if not evidence_after_sha:
        raise SystemExit("Health check failed: deployment evidence is missing after_sha for parity comparison.")
    if runtime_build_revision != evidence_after_sha:
        raise SystemExit(
            "Health check failed: deployment runtime_build_revision "
            f"{runtime_build_revision!r} does not match deployment evidence after_sha {evidence_after_sha!r}."
        )

index_request = urllib.request.Request(f"{base_url}/", method="GET")
with urllib.request.urlopen(index_request, timeout=30) as response:
    web_index_html = response.read().decode("utf-8")
...
if match is None:
    raise SystemExit("Health check failed: web shell build revision meta tag is missing from '/'.")
web_build_revision = match.group("revision").strip()
if not web_build_revision:
    raise SystemExit("Health check failed: web shell build revision is empty.")
if web_build_revision != runtime_build_revision:
    raise SystemExit(
        "Health check failed: web shell build revision "
        f"{web_build_revision!r} does not match deployment runtime_build_revision {runtime_build_revision!r}."
    )
...
summary = {
    ...
    "deployment_runtime_build_revision": runtime_build_revision,
    "web_shell_build_revision": web_build_revision,
    "deployment_local_repo_head_sha": local_repo_head_sha,
